...
% MAIN SCRIPT: Thermally Choked Validation Case (Case b)
%
% Description:
%   Runs the full RAMJET flow path analysis for Case b, where M2 = 0.40
%   causes the combustor to become thermally choked (M3 = 1 at combustor
%   exit, capping maximum heat addition).
%
% Key difference from Case a:
%   M2 = 0.40 (vs 0.15 in Case a) --> higher M at combustor inlet
%   --> lower Tt3_choked --> combustor chokes before reaching Tt3_max
%   --> less net work output, reduced efficiency and thrust
%
% How to run:
%   >> main_choked
...
fprintf('  Case: THERMALLY CHOKED (M2 = 0.40)\n');
fprintf('=============================================================\n\n');
...
z_star   = 8404;    % Reference altitude [m]
M1       = 2.4;     % Flight Mach number [-]
M2       = 0.40;    % Diffuser exit Mach number [-]  <-- HIGHER: induces thermal choking
Tt3_max  = 2400;    % Max allowable combustor exit temperature [K]
Ae       = 0.015;   % Nozzle exit area [m²]
eta_d    = 0.92;    % Diffuser efficiency [-]
eta_n    = 0.94;    % Nozzle efficiency [-]
qf       = 43.2e6;  % Fuel heating value [J/kg]
...
if M3 == 1
    fprintf('>> STATUS: Combustor is THERMALLY CHOKED (M3 = 1, Tt3 limited to %.1f K)\n\n', Tt3);
else
    fprintf('>> STATUS: Combustor is NOT thermally choked (M3 = %.4f)\n\n', M3);
end
